...
from system.settings import CACHE_FOLDER,NO_IMAGE_PLACEHOLDER,WATERMARK
import os
from django.utils.translation import gettext_lazy as _

try:
    from system.settings import WOTERMARK_SIZE, WOTERMARK_OPACITY
except:
    WOTERMARK_SIZE = None
...
class AbstractImage(models.Model): 
    ...
    image = models.ImageField(max_length=255,upload_to='products/%Y/%m/%d', blank=True,verbose_name=_('Зображення'))
    position = models.PositiveIntegerField(default=0,verbose_name=_('Позіція в списку'),blank=True)
    ...
    def __getattr__(self,name):
        if name in self.size:
            try:
                ...
                if thumb.url and os.path.isfile(CACHE_FOLDER + thumb.url):
                    return thumb.url
                elif thumb.url:
                    try:
    ...
    def thumbnail(self,size,path = None):
        ...
        try:
            image.save(CACHE_FOLDER[:-1] + path,'JPEG',quality=80)
    ...
    def last(self):
        positions = sorted({i.id:i.position for i in Product.objects.get(id=self.product_id).Image.all()}.items(), key=lambda x: (x[1] is None, x[1]), reverse=True)
...
class Thumb(models.Model): 
    ...
    image = models.ForeignKey(Image, related_name='thumb', on_delete=models.CASCADE)
    size = models.PositiveIntegerField()

    class Meta:
        verbose_name = _('Мініатюра зображення')
        verbose_name_plural = _('Мініатюри зображення')
