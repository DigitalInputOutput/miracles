...
from checkout.models import Order,Cart
...
class CheckoutForm(forms.ModelForm): 
    ...
    def clean(self):
        data = super().clean()
        if not 'delivery' in data or data['delivery'] != '4':
            # if not data['sname']:
        ...
        if not 'delivery' in data or data['delivery'] == '3' and not data['address']:
            raise forms.ValidationError({'address':_('Обязательное поле')})

        if not 'delivery' in data or data['delivery_type'] == '5' and not data['address']:
            raise forms.ValidationError({'address':_('Обязательное поле')})
    ...
    class Meta:
        model = Order
        fields = ('address','delivery','payment','comment','email','phone','name','lname','sname')
