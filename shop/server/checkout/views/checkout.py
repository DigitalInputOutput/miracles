...
from checkout.views import Buyer
...
from checkout import strings

try:
    from system.settings import SMS_ACCOUNT,SMS_PASSWORD,SMS_SIGN
except:
...
class CheckoutView(View): 
    def get(self,request,*args,**kwargs):
        ...
        context = {
            ...
            'title':_(strings.checkout_title),
            'view':'Checkout',
            'cartJson':cart.dict
        }
...
class OrderView(View): 
    ...
    def post(self,request,*args,**kwargs):
        ...
        if form.is_valid():
            ...
            messages.success(request, _(strings.checkout_success))

            if request.user.is_authenticated:
                return redirect(f'/{request.LANGUAGE_CODE}/user/order/{order.id}')
            else:
                return redirect(f'/{request.LANGUAGE_CODE}')
        ...
        return render(request, f'checkout/{request.folder}/checkout.html', context)
