...
export class Filter{
    constructor(parameters){
        ...
        Dom.query('#filter').on('click',this.filter.bind(this));
        ...
        if(scroll){
            ...
            GET(href, {
                success: (response) => {
                    ...
                    if(scroll && that.page){
                        Dom.query('#category .items').after(response.responseText);
                        ...
                        Dom.query('#category .items').html(response.responseText);
                    }
    ...
    scrollFilter(event){
        let scroll = document.body.clientHeight - window.scrollY - window.screen.height;
        if(scroll > 7000)
            return;
    ...
    slider(){
        if(this.min && this.max && (this.min < this.max)){
            let slider = Dom.query('#slider-range');
            let from = Dom.query("#amount-from");
            let to = Dom.query("#amount-to");
